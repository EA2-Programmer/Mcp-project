...
async def lifespan(app: FastAPI):
    ...
    # 1. Load System Prompt – use Path to be safe
    prompt_path = Path(__file__).parent / "config/prompts/system_prompt.md"
    try:
...
async def health():
    return {
        "status": "ok",
        "mcp_connected": mcp_manager.process is not None if mcp_manager else False
    }
...
async def get_traces(limit: int = 80):
    try:
        ...
        langfuse_url = settings.langfuse_base_url.rstrip('/')
        traces_url = f"{langfuse_url}/api/public/traces"

        async with httpx.AsyncClient() as client:
            response = await client.get(
                traces_url,
                headers={"Authorization": f"Basic {creds}"},
                params={"limit": limit, "orderBy": "timestamp.desc", "include": "observations"},
                timeout=10.0
            )
            response.raise_for_status()
            return response.json()
    except httpx.HTTPStatusError as e:
        logger.error(f"Langfuse error: {e.response.status_code} - {e.response.text}")
        return {"error": f"Langfuse error: {e.response.status_code}", "details": e.response.text, "data": []}
    except Exception as e:
