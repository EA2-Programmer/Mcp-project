    ...
    for recipe in recipes:
        ...
        sql_batches = """
            SELECT TOP 10
                ...
                b.State             AS state,
                CONVERT(nvarchar(30), b.StartDateTime, 127) AS batch_start,
                CONVERT(nvarchar(30), b.EndDateTime,   127) AS batch_end,
                j.ID                AS job_id,
                j.Name              AS job_name,
                s.Name              AS system_name
            FROM tJobBatch jb
    ...
    if include_batch_count:
        sql = f"""
            SELECT TOP {safe_limit}
                ...
                MAX(CONVERT(nvarchar(30), b.StartDateTime, 127)) AS last_batch_date
            FROM tProduct p
